...
#             perspect.append(perspective_transform(i, ratio))
            ...
            for s1 in perspect:
                gauss_noise = [] #1
                high = 37 if index<threshold else 8
                        ...
                        for r in rotate1:
                            ...
                            for image in final:
                                ...
                                if s > 150 and s<=200:
                                    ...
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
#                                     ratio = np.random.uniform(low=0.35, high=0.55)
#                                     final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                if s > 200 and s<=250:
                                    ratio = np.random.uniform(low=0.125, high=0.307)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                ...
                                for finalImage in final2:
                                    cv2.imwrite('3/'+str(count)+'.jpg', finalImage)
                                    count+=1
                                    if count%50==0:
                                        print('count: ', count)
