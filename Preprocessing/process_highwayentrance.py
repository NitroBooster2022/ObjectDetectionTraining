...
#             perspect.append(perspective_transform(i, ratio))
            ...
            for s1 in perspect:
                gauss_noise = [] #1
                high = 37 if index<threshold else 2
...
#                             print("shapeR, shape: ", shapeR, gb.shape)
                        ...
                        for r in rotate1:
                            ...
                            final.append(cv2.convertScaleAbs(r, alpha=0.37, beta=10))
                            ...
                            for image in final:
                                ...
                                if s > thresh and s<=150:
                                    ratio = np.random.uniform(low=0.28, high=0.68)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                    ratio = np.random.uniform(low=0.28, high=0.69)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                if s > 150 and s<=200:
                                    ratio = np.random.uniform(low=0.1, high=0.52)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                    ratio = np.random.uniform(low=0.1, high=0.52)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                if s > 200 and s<=250:
#                                     ratio = np.random.uniform(low=0.40, high=0.7)
#                                     final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                    ratio = np.random.uniform(low=0.06, high=0.42)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                if s > 250 and s<=300:
                                    ratio = np.random.uniform(low=0.055, high=0.35)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                if s > 300 and s<=350:
                                    ratio = np.random.uniform(low=0.04, high=0.3)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                if s > 350:
                                    ratio = np.random.uniform(low=0.035, high=0.29)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                for finalImage in final2:
                                    cv2.imwrite('1/'+str(count)+'.jpg', finalImage)
                                    count+=1
                                    if count%50==0:
                                        print('count: ', count)

name = 'highwayentrance'
