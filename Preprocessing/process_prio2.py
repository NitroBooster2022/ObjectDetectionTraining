...
#                             ratio = np.random.uniform(low = 0.7, high = 0.8)
#                             rotate1.append(perspective_transform(gb, ratio))
#                         else:
...
#                             final.append(cv2.convertScaleAbs(r, alpha=r2, beta=0))
                            ...
                            final.append(cv2.convertScaleAbs(r, alpha=0.42, beta=10))
...
#                                     final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                    ratio = np.random.uniform(low=0.28, high=0.65)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                if s > 150 and s<=200:
                                    ratio = np.random.uniform(low=0.12, high=0.46)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
#                                     ratio = np.random.uniform(low=0.45, high=0.65)
#                                     final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                if s > 200 and s<=250:
                                    ratio = np.random.uniform(low=0.085, high=0.347)
                                    final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
#                                     ratio = np.random.uniform(low=0.3, high=0.5)
#                                     final2.append(cv2.resize(image,(int(image.shape[1]*ratio), int(image.shape[0]*ratio))))
                                if s > 250 and s<=300:
                                ...
                                for finalImage in final2:
                                    cv2.imwrite('8/'+str(count)+'.jpg', finalImage)
                                    count+=1
                                    if count%50==0:
                                        print('count: ', count)
